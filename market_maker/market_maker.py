...
class OrderManager:
    ...
    def short_position_limit_exceeded(self,orders):
        """Returns True if the short position limit is exceeded"""
        if not settings.CHECK_POSITION_LIMITS:
            return False
        position = self.exchange.get_delta()-sum(item['orderQty'] for item in orders)
        return position <= settings.MIN_POSITION

    def long_position_limit_exceeded(self,orders):
    ...
    def prepare_order(self, index, aggressive=False):
        ...
        price=0
        if aggressive and (index ==1 or index==-1):
            price=(self.start_position_mid-self.instrument['tickSize'] if index< 0 else self.start_position_mid+self.instrument['tickSize'])
        else:
    ...
    def run_loop(self):
        ...
        while True:
            ...
            self.exchange.sleep(settings.LOOP_INTERVAL)
            ...
            if not self.check_connection():
                ...
                self.restart()
            if not self.exchange.check_market_open():
                break
            self.sanity_check()  # Ensures health of mm - several cut-out points here
            # self.print_status()  # Print skew, delta, etc
            # if(i%100==0):
            #     print(self.exchange.get_current_time())
                # logger.info(self.exchange.buy_orders)
                # logger.info(self.exchange.sell_orders)
                # logger.info("maitreya")
